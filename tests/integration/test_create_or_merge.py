"""Integration tests for create_or_merge functionality.

Tests the tuple unpacking fix for _create_node, _merge_node,
_create_edge, and _merge_edge on both Memgraph and Apache AGE backends.
"""
...
@pytest.fixture(params=["memgraph_db", "age_db"])
def test_db(request):
...
class TestCreateNode:
    ...
    def test_create_node_assigns_integer_id(self, test_db, test_node_model):
        """Test that creating a node assigns an integer ID, not a tuple."""
        # Create the node
        created_node = test_db.create_or_merge(test_node_model, strategy="force_create")
    ...
    def test_create_node_preserves_properties(self, test_db, test_node_model):
    ...
    def test_create_multiple_nodes_get_different_ids(self, test_db):
        """Test that creating multiple nodes assigns different IDs."""
        node1 = PersonNode(name="Person1", age=25)
        node2 = PersonNode(name="Person2", age=30)

        created1 = test_db.create_or_merge(node1, strategy="force_create")
        created2 = test_db.create_or_merge(node2, strategy="force_create")
...
class TestMergeNode:
    ...
    def test_merge_node_updates_properties(self, test_db, test_node_model):
        ...
        merged_node = test_db.create_or_merge(created_node, strategy="merge")
    ...
    def test_merge_node_fetches_latest_from_db(self, test_db):
        """Test that merge fetches the latest node state from database."""
        # Create a node
        test_node = PersonNode(name="TestUser", age=25)
        created = test_db.create_or_merge(test_node, strategy="force_create")
        node_id = created.id_

        # Update directly in database
        test_db.execute(f"MATCH (n) WHERE id(n) = {node_id} SET n.age = 99, n.email = 'db@example.com'")

        # Now merge - should fetch updated values from DB
        created.email = "will_be_overwritten@example.com"
        merged = test_db.create_or_merge(created, strategy="merge")
...
class TestCreateEdge:
    ...
    def test_create_edge_assigns_integer_id(self, test_db, test_edge_model):
        """Test that creating an edge assigns an integer ID, not a tuple."""
        test_edge, _, _ = test_edge_model

        # Create the edge
        created_edge = test_db.create_or_merge(test_edge, strategy="force_create")
    ...
    def test_create_edge_preserves_properties(self, test_db, test_edge_model):
...
class TestMergeEdge:
    ...
    def test_merge_edge_updates_properties(self, test_db, test_edge_model):
        ...
        merged_edge = test_db.create_or_merge(created_edge, strategy="merge")
...
class TestCreateOrMergeStrategy:
    ...
    def test_force_create_always_creates_new(self, test_db):
...
class TestRoundTrip:
    ...
    def test_node_roundtrip(self, test_db):
        ...
        updated = test_db.create_or_merge(fetched, strategy="merge")
    ...
    def test_edge_roundtrip(self, test_db):
        ...
        # Create edge
        test_edge = RelationEdge(start_id_=node1.id_, end_id_=node2.id_, since=2019, role="friend")
        created_edge = test_db.create_or_merge(test_edge, strategy="force_create")
        ...
        # Fetch by ID
        fetched_edge = test_db.fetch_edges(edge_id, unnest_result="rc")
        ...
        updated_edge = test_db.create_or_merge(fetched_edge, strategy="merge")
        ...
        # Fetch again
        final_edge = test_db.fetch_edges(edge_id, unnest_result="rc")
