"""YAML file importer using hierarchical import logic.

Provides a file-specific wrapper for YAML import functionality that integrates
...
class YamlImporter(FileImporter):
    """YAML file importer for graph data (hierarchical processing).

    This importer handles YAML files containing nested graph data structures.
    ...
    def load_from_file(self, filename: str):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to import YAML file {filename}: {ex}") from ex

    def load(self, file_or_dirname: str, recursive: bool = False) -> list[str]:
        """Load YAML data from file(s) or directory.
